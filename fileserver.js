...
function getCurrentTransit(date, time, weekday, callback){
    ...
    sys.puts('Query on getCurrentTransit : ', date, time, weekday);
...
function getRouteShape(SHAPE_ID, callback){
    sys.puts('Query on getRouteShape: ', SHAPE_ID);
    var queryString = "select SHAPE_PT_LAT, SHAPE_PT_LON, SHAPE_PT_SEQUENCE from shapes WHERE SHAPE_ID='" + SHAPE_ID + "' order by SHAPE_PT_SEQUENCE ";
    sys.puts(queryString);
    ...
    if (request.method == 'POST'){
        ...
        request.on('end', function(){
            ...
            if (my_path.indexOf("getCurrentTransit") > -1){
                getCurrentTransit(post['date'], post['time'], post['weekday'], retJson);    
            }else if (my_path.indexOf("getRouteShape") > -1){
                getRouteShape(post['SHAPE_ID'], retJson);
            }else{
